...
SELECT  ENAME, A.DEPTNO, DNAME --칼럼이름이 자동으로 설정되지만 직접 칼럼명을 명시하면 두 테이블중 
--어디인지 헷갈려서 어디 테이블에 있는 칼럼인지 명시를 해줘야 된다.
FROM EMP A
LEFT OUTER JOIN DEPT B
ON A.DEPTNO =B.DEPTNO ;--ANSI 표준

INSERT INTO EMP (EMPNO , ENAME , DEPTNO ) VALUES (999, '홍길동', NULL);
--INNER JOIN 사용시 홍길동 안나옴
--OUTER JOIN(LEFT (OUTER 생략가능) JOIN) 사용시 홍길동 나옴
